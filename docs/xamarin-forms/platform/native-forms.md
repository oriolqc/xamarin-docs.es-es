...
- El `FolderPath` propiedad se inicializa en una ruta de acceso en el dispositivo donde se almacenarán los datos de la nota.
...
[![Captura de pantalla de una aplicación de Xamarin.iOS que usa una interfaz de usuario definido en XAML](native-forms-images/ios-notespage.png "aplicación de Xamarin.iOS con una UI XAML")](native-forms-images/ios-notespage-large.png#lightbox "aplicación de Xamarin.iOS con una UI de XAML")

Interactuar con la interfaz de usuario, por ejemplo, al pulsar la **+** [ `Button` ](xref:Xamarin.Forms.Button), dará como resultado el siguiente controlador de eventos en el `NotesPage` ejecutando el código subyacente:
...
[![Captura de pantalla de una aplicación de Xamarin.iOS que usa una interfaz de usuario definido en XAML](native-forms-images/ios-noteentrypage.png "aplicación de Xamarin.iOS con una UI XAML")](native-forms-images/ios-noteentrypage-large.png#lightbox "aplicación de Xamarin.iOS con una UI de XAML")
...
> [!WARNING]
> El Popping de un `UIViewController` en la navegación nativo de iOS pila no eliminará automáticamente `UIViewController`s. Es responsabilidad del programador asegurarse de que cualquier `UIViewController` que ya no se necesita tiene su `Dispose()` llama al método, lo contrario, el `UIViewController` y adjunta `Page` quedarán huérfanos y no se recopilarán por el recolector de elementos no utilizados lo que produce una pérdida de memoria.
...
[![Captura de pantalla de una aplicación de Xamarin.Android que usa una interfaz de usuario definido en XAML](native-forms-images/android-notespage.png "aplicación de Xamarin.Android con una UI XAML")](native-forms-images/android-notespage-large.png#lightbox "aplicación de Xamarin.Android con una UI de XAML")
...
[![Captura de pantalla de una aplicación de Xamarin.Android que usa una interfaz de usuario definido en XAML](native-forms-images/android-noteentrypage.png "aplicación de Xamarin.Android con una UI XAML")](native-forms-images/android-noteentrypage-large.png#lightbox "aplicación de Xamarin.Android con una UI de XAML")
...
### <a name="choose-a-file"></a>Elija un archivo
...
[![Captura de pantalla de una aplicación de UWP que usa una interfaz de usuario definida con XAML de Xamarin.Forms](native-forms-images/uwp-notespage.png "aplicación para UWP con una interfaz de usuario de XAML de Xamarin.Forms")](native-forms-images/uwp-notespage-large.png#lightbox "aplicación para UWP con una interfaz de usuario de XAML de Xamarin.Forms")
...
[![Captura de pantalla de una aplicación de UWP que usa una interfaz de usuario definida con XAML de Xamarin.Forms](native-forms-images/uwp-noteentrypage.png "aplicación para UWP con una interfaz de usuario de XAML de Xamarin.Forms")](native-forms-images/uwp-noteentrypage-large.png#lightbox "aplicación para UWP con una interfaz de usuario de XAML de Xamarin.Forms")
...
El `OnBackRequested` llamadas del controlador de eventos el `GoBack` método en el marco de la raíz de la aplicación y establece el `BackRequestedEventArgs.Handled` propiedad a `true` para marcar el evento como controlado. Error al marcar el evento como controlado podría producir en el evento que se pasa por alto.

La aplicación decide si se deben mostrar un botón Atrás en la barra de título. Esto se consigue estableciendo la `AppViewBackButtonVisibility` propiedad en uno de los `AppViewBackButtonVisibility` valores de enumeración:
