...
ms.locfileid: "53052114"
---
# <a name="automate-xamarinforms-testing-with-app-center"></a>Automatización de pruebas de Xamarin.Forms con App Center

[![Descargar ejemplo](~/media/shared/download.png) Descargar el ejemplo](https://developer.xamarin.com/samples/xamarin-forms/UsingUITest/)
...
La propiedad [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) también se puede establecer en XAML:
...
> [!NOTE]
> El elemento [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) corresponde a un valor [`BindableProperty`](xref:Xamarin.Forms.BindableProperty) y también se puede establecer con una expresión de enlace.

Se debe agregar un valor [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) único a todos los controles que sean necesarios para las pruebas (como botones, entradas de texto y etiquetas cuyo valor es posible que se necesite consultar).

> [ADVERTENCIA] Tenga en cuenta que se producirá una excepción `InvalidOperationException` si se trata de establecer la propiedad [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) de un elemento [`Element`](xref:Xamarin.Forms.Element) más de una vez.
...
Antes de poder escribir cualquier UITest, la interfaz de usuario de la aplicación de Xamarin.Forms debe permitir ejecutar scripts. Asegúrese de que todos los controles de la interfaz de usuario tengan un elemento [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId)para que se pueda hacer referencia a ellos en el código de prueba.

#### <a name="referring-to-the-automationid-in-uitests"></a>Hacer referencia a AutomationId en UITests

Al escribir UITests, el valor [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) se expone de forma distinta en cada plataforma:

- **iOS** usa el campo `id`.
- **Android** usa el campo `label`.

Para escribir UITests multiplataforma que encuentren [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) en iOS y Android, use la consulta de prueba `Marked`:
...
1. Desde las plantillas **Visual C#**, seleccione la categoría **Prueba**. Seleccione la plantilla **Aplicación de prueba de interfaz de usuario > Multiplataforma**:
    ...
    ![Agregar nuevo proyecto](uitest-and-test-cloud-images/08-new-uitest-project-vs.png "Add New Project")

    Esto agregará un nuevo proyecto con los paquetes de NuGet **NUnit**, **Xamarin.UITest** y **NUnitTestAdapter** a la solución:

    ![Administrador de paquetes NuGet](uitest-and-test-cloud-images/09-new-uitest-project-xs.png "NuGet Package Manager")
...
1. Agregue una referencia de proyecto desde el proyecto de UITest al proyecto de Xamarin.Android:
    ...
    ![Administrador de referencias de proyecto](uitest-and-test-cloud-images/10-test-apps-vs.png "Project Reference Manager")
...
1. Empiece agregando un nuevo proyecto. Para ello, seleccione la solución y haga clic en **Archivo > Agregar nuevo proyecto**. En el cuadro de diálogo **Nuevo proyecto**, seleccione **Multiplataforma > Pruebas > Xamarin Test Cloud > Aplicación de prueba de interfaz de usuario**:
    ...
    ![Elija una plantilla](uitest-and-test-cloud-images/02-new-uitest-project-xs.png "Choose a Template")

    Esto agregará un nuevo proyecto que ya tiene los paquetes de NuGet **NUnit** y **Xamarin.UITest** en la solución:

    ![Paquetes NuGet UITest de Xamarin](uitest-and-test-cloud-images/03-new-uitest-project-xs.png "Xamarin UITest NuGet Packages")
...
1. Seleccione **Vista > Paneles > Pruebas unitarias** para mostrar el panel de pruebas unitarias. Expanda **UsingUITest > UsingUITest.UITests > Aplicaciones de prueba**:
    ...
    ![Panel de prueba unitaria](uitest-and-test-cloud-images/04-unit-test-pad-xs.png "Unit Test Pad")
...
1. Haga clic con el botón derecho en **Aplicaciones de prueba**, haga clic en **Agregar proyecto de aplicación** y seleccione proyectos de iOS y Android en el cuadro de diálogo que aparece:
    ...
    ![Cuadro de diálogo para probar aplicaciones](uitest-and-test-cloud-images/05-add-test-apps-xs.png "Test Apps Dialog")
...
1. Agregue el paquete NuGet **Xamarin Test Cloud Agent**. Haga clic con el botón derecho en **Paquetes**, seleccione **Agregar paquetes**, busque **Xamarin Test Cloud Agent** en NuGet y agréguelo al proyecto de Xamarin.iOS:
    ...
    ![Agregar paquetes NuGet](uitest-and-test-cloud-images/07-add-test-cloud-agent-xs.png "Add NuGet Packages")
...
Las aplicaciones de Xamarin.Forms se pueden probar fácilmente con **Xamarin.UITest** mediante un mecanismo sencillo para exponer el elemento [`AutomationId`](xref:Xamarin.Forms.Element.AutomationId) como identificador de vista único para la automatización de pruebas. Una vez que se ha agregado un proyecto de UITest a una solución de Xamarin.Forms, los pasos para escribir y ejecutar las pruebas para una aplicación de Xamarin.Forms son los mismos que para una aplicación de Xamarin.Android o Xamarin.iOS.
