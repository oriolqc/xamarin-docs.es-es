...
El método tiene acceso a la `MainActivity` clase para varias cosas: para la `PickImageId` field, para la `TaskCompletionSource` propiedad y para llamar a `StartActivityForResult`. Este método se define mediante la `FormsApplicationActivity` clase que es la clase base de `MainActivity`.
