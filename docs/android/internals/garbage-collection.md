...
Si pasa de una inicial dispose causas `childViews` que no es válido `Handle`, el `for` acceso de bucle producirá una `ArgumentException`. Agregando explícita `Handle` null verificación antes del primer `childViews` tener acceso a la siguiente `Dispose` método evita que la excepción ocurre: 
