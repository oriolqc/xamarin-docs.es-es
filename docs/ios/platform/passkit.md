...
description: La aplicación Wallet permite a los usuarios almacenar pases digitales en sus dispositivos de iOS. El marco de PassKit permite a los programadores interactuar con pases mediante programación.
ms.prod: xamarin
...
La aplicación de iOS Wallet permite a los usuarios almacenar pases digitales en sus dispositivos.
Estos pases se generan por los comercios y se envían al cliente por correo electrónico, direcciones URL, o a través de la aplicación de iOS compañera (su app). Estos pases pueden representar varias cosas, de vales de película a tarjetas de fidelización de embarque. El marco de desarrollo de PassKit permite a los programadores interactuar con pases mediante programación.
...
-   **Compra de vales de película en línea** : normalmente, los clientes reciben un correo electrónico un código de barras que representa los boletos. Este código de barras se imprime y muestra en el cine para analizarse en la taquilla.
-   **Tarjetas de fidelización** : los clientes llevan un número de tarjetas específicas de almacenes diferentes en su cartera o el bolso, para mostrar y de análisis cuando compran productos.
-   **Cupones** – cupones que se distribuyen a través de correo electrónico, como páginas web imprimible, a través de los buzones de correo y como códigos de barras en periódicos y revistas. Los clientes pueden llevarlos a un almacén para el análisis, para recibir  devolución de mercancía, servicios o descuentos.
...
-   **Vales de película** : después de la compra, el cliente agrega un pase con los datos de el evento (a través de correo electrónico o un vínculo de sitio Web). Como la hora de la de película, la fase de aparecerán automáticamente en la pantalla de bloqueo como recordatorio, y a la llegada en el cine el pase fácilmente se recupera y muestra en la taquilla para el análisis.
-   **Tarjetas de fidelización** : en lugar de (o además) proporcionar una tarjeta física, almacenes pueden emitir (por correo electrónico o después de un inicio de sesión del sitio Web) un pase de la tarjeta de el almacén. El almacén puede proporcionar características adicionales, como actualizar el saldo de la cuenta en el paso a través de notificaciones de inserción, y utilizando los servicios de ubicación geográfica el pase podría aparecer automáticamente en la pantalla de bloqueo cuando el cliente está cerca de una ubicación de almacén.
-   **Cupones** – Cupones fácilmente se pueden generar con características únicas para ayudar a con el seguimiento y distribuidas a través de vínculos de correo electrónico o un sitio Web. Cupones descargados pueden aparecer automáticamente en la pantalla de bloqueo cuando el usuario está cerca de una ubicación específica, o en una fecha determinada (por ejemplo, cuando se está aproximando la fecha de expiración). Dado que los cupones se almacenan en el teléfono del usuario, que siempre son útiles y no se traspapelan. Cupones podrían anime a los clientes para descargar aplicaciones complementarias porque los vínculos de App Store se pueden incorporar en el pase, aumentar la interacción con el cliente.
-   **Pases de abordar** – después de un proceso en línea en el repositorio, el cliente recibiría su pase de abordar a través de correo electrónico o un vínculo. Una aplicación complementaria proporcionada por el proveedor de transporte podría incluir el proceso de comprobación y también permiten al cliente realizar funciones adicionales como la elección de su asiento o comida. El proveedor de transporte puede usar notificaciones de inserción para actualizar el paso si el transporte se retrasa o cancelado. Como los métodos de tiempo de embarque el pase aparecerá en la pantalla de bloqueo como recordatorio y para proporcionar acceso rápido al pase.
...
-   **Wallet** : aplicación de iOS integrada de Apple que almacena y muestra pases. Esto es el único lugar en que se representan pases para su uso en el mundo real (es decir el código de barras se muestra, junto con todos los datos localizados en la fase).
-   **Aplicacion compañera (Su app)** : Aplicaciones de iOS 6 creadas por proveedores de pases para extender la funcionalidad de los pases a emitir, como agregar valor a una tarjeta de almacén, cambiar la plaza en un paso de embarque u otra función específicos del negocio. Aplicaciones complementarias no son necesarias para un pase para ser útil.
-   **El servidor** : un servidor seguro donde el pase puede ser generado y firmado para la distribución. Su aplicación complementaria puede conectarse al servidor para generar nuevos pases o solicitar actualizaciones a pases existentes. También puede opcionalmente, implementar la web API del servicio que llamaría Wallet para actualizar el pase.
-   **Servidores de Apple Push Notification Service** : el servidor tiene la capacidad para notificar a Wallet de actualizaciones de un pase en un dispositivo determinado con APNS. Una notificación de inserción a Wallet que, a continuación, pondremos en contacto con el servidor para obtener detalles del cambio. No es necesario implementar APNS para esta característica aplicaciones complementarias (pueden escuchar la `PKPassLibraryDidChangeNotification` ).
-   **Conduit Apps** : las aplicaciones que no se manipulan directamente el pase (como ocurre con aplicaciones complementarias), pero que puede mejorar su utilidad al reconocer pases y permitir que se agreguen a Wallet. Los clientes de correo electrónico, exploradores de redes sociales y otras aplicaciones de la agregación de datos pueden todos encontrar vínculos a los pases o datos adjuntos.
...
-  **Boleto de evento** – Con un pequeño recorte semicircular.
-  **Pase de abordar** – muescas en los costados, se pueden especificar el transporte  (p ej. bus, tren, avión).
-   **Tarjeta de almacén** : redondeado en los bordes, como una tarjeta de crédito o débito.
-  **Cupón** : perforado a lo largo de la parte superior.
-  **Genérico** : igual que la tarjeta de almacén, la parte superior redondeada.


Los tipos de cinco pases se muestran en esta captura de pantalla (en orden: cupón, genérico, tarjeta de almacén, pase de abordar y boleto de eventos):
...
Un archivo de pase es realmente un archivo ZIP con una extensión **.pkpass** , que contiene algunos específicos archivos JSON (obligatorios), una variedad de archivos de imagen (opcional), así como las cadenas localizadas (también opcional).

-   **PASS.JSON** : requerido. Contiene toda la información de la pase.
-   **manifest.JSON** : requerido. Contiene el hash de SHA1 para cada archivo en el pase, exceptuando el archivo de firma y este archivo (manifest.json).
...
A continuación se muestra la estructura de directorios de un archivo de pase (este es el contenido del archivo ZIP):
...
Hay un gran número de otras claves JSON en cada pase, se muestra un ejemplo de los cuales a continuación:
...
Se pueden especificar hasta 10 ubicaciones en un pase, por ejemplo, los almacenes que un cliente visita con frecuencia, o la ubicación de un cine o un aeropuerto. Un cliente puede especificar estas ubicaciones a través de una aplicación complementaria o el proveedor podría determinar ellos desde los datos (si se recopilan con el permiso del cliente).

Cuando el pase se muestra en la pantalla de bloqueo, se calcula una barrera para que cuando el usuario abandona el área de la fase está oculto en la pantalla de bloqueo. Está asociado el radio para pasar de estilo para evitar abusos.

 **Fecha y hora**

En un pase, se puede especificar solo una fecha y hora. La fecha y hora es útil para la activación de los avisos de pantalla de bloqueo de embarque y vales de eventos.
...
Los pases se firman con un certificado privado que genera en el Portal de aprovisionamiento de iOS. Los pasos para firmar el pase son:

1.  Calcular un hash SHA1 para cada archivo en el directorio de el pase (no incluya el `manifest.json` o `signature` archivo, ninguno de los cuales debe existir en esta fase de todos modos).
...
Dado que la clave privada se necesita para firmar el pase, este proceso solo debe realizarse en un servidor seguro que usted controla. No distribuya las claves para probar y generar pases en una aplicación.
...
Para que un pase pueda ingresar en la App Store, se debe vincular a una cuenta de desarrollador. Esto requiere dos pasos:

1.  El debe estar registrada con un identificador único, llamado el identificador del tipo de pase.
1.  Para iniciar el pase con firma digital del desarrollador, se debe generar un certificado válido.

Para crear un Pass Type ID realice lo siguiente.

#### <a name="create-a-pass-type-id"></a>Crear un identificador de tipo de paso

El primer paso es configurar un Pass Type ID para cada pase que será soportado. El Pass Type ID (o Pass Type identifier) crea un identificador único para cada pase. Este identificador se usará para vincular el pase con su cuenta de desarrollador con un certificado.

1. En el [certificados, identificadores y perfiles de sección del Portal de aprovisionamiento de iOS](https://developer.apple.com/account/overview.action), vaya a **identificadores** y seleccione **pasar los identificadores de tipo** . A continuación, seleccione el **+** botón para crear un nuevo tipo de pase: [ ![](passkit-images/passid.png "crear un nuevo tipo de pase")](passkit-images/passid.png#lightbox)

2.   Proporcione un **descripción** (nombre) y **identificador** (cadena única) para el pase. Tenga en cuenta que todos los identificadores de tipo pasar debe comenzar con la cadena `pass.` en este ejemplo usamos `pass.com.xamarin.coupon.banana` : [ ![](passkit-images/register.png "proporcionar una descripción y un identificador")](passkit-images/register.png#lightbox)
...
Ahora que hemos creado un certificado para este Pass Type ID, la siguiente sección describe cómo crear un paso manualmente.
...
Ahora que hemos creado el tipo de pase manualmente podemos crear un paso de prueba en el simulador o en un dispositivo. Los pasos para crear un paso son:

-  Cree un directorio que contenga los archivos de pase.
...
-  Comprima el contenido del directorio y cambie el nombre con extensión .pkpass.


Hay algunos archivos de origen en el [código de ejemplo](https://developer.xamarin.com/samples/monotouch/PassKit/) para este artículo que se puede usar para generar un pase. Use los archivos en el `CouponBanana.raw` directorio del directorio CreateAPassManually. Los siguientes archivos están presentes:
...
Descargue el **materiales de soporte de semillas de Wallet** desde el [descargas Apple](https://developer.apple.com/downloads/index.action?name=Passbook) sitio. Use el `signpass` herramienta para convertir la carpeta en un pase (Esto también calculará SHA1 aplica un algoritmo hash y comprimir la salida en un archivo .pkpass).
...
Wallet es la pieza central del ecosistema de PassKit. Esta captura de pantalla muestra Wallet vacío y el aspecto de la lista de pases y pases individuales:

 [![](passkit-images/image21.png "Esta captura de pantalla muestra Wallet vacío y el aspecto de la lista de pases y pases individuales")](passkit-images/image21.png#lightbox)

Las características de Wallet incluyen:

-  Es el único lugar donde los pases se representan con su código de barras para el análisis.
-  Usuario puede cambiar la configuración de actualizaciones. Si está habilitada, las notificaciones de inserción pueden desencadenar actualizaciones a los datos en el pase.
-  Usuario puede habilitar o deshabilitar la integración de la pantalla de bloqueo. Si está habilitada, esto permite el paso aparecen automáticamente en su pantalla de bloqueo, según los datos de hora y la ubicación pertinentes incrustados en el pase.
...
-  Los pases pueden eliminarse (con una animación de purga bonita).

## <a name="adding-passes-into-wallet"></a>Agregar fases de Wallet

Pueden agregarse pases a Wallet de las maneras siguientes:

* **Conduit Apps** : estos no manipulan directamente los pases, simplemente cargar archivos de pase y muestran al usuario la opción de agregarlo a Wallet. 

* **Companion en aplicaciones** : estos se escriben proveedores para distribuir pases y ofrecer una funcionalidad adicional para examinar o modificarlos. Las aplicaciones de Xamarin.iOS tienen acceso completo a la API de PassKit para crear y manipular pasadas. A continuación, se pueden agregar pases al uso de Wallet el `PKAddPassesViewController`. Este proceso se describe con más detalle en la **aplicaciones complementarias** sección de este documento.

### <a name="conduit-applications"></a>Aplicaciones de la canalización

Aplicaciones conducto son aplicaciones intermedias que pueden aparecer pases en el nombre de un usuario y deben programarse para que reconozca su tipo de contenido y proporcionan funcionalidad para agregar a Wallet. Ejemplos de aplicaciones de la canalización:

-   **Correo electrónico** – reconoce los datos adjuntos como un pase.
-   **Safari** – reconoce el Content-Type del pase al que se hace clic en un vínculo de dirección URL.
...
Esta captura de pantalla se muestra cómo **correo** en iOS 6 reconoce un adjunto de pase y (cuando se toca) ofrece la posibilidad de **agregar** a Wallet.
...
Si va a compilar una aplicación que podría ser un conducto para pases, puede reconocer por:
...
Una aplicación complementaria proporciona funcionalidad adicional para trabajar con pases, incluyendo la creación de un pase, actualizar la información asociada con un pase y administrar en caso contrario pasadas asociadas a la aplicación.

Las aplicaciones complementarias no deben intentar duplicar las características de Wallet. No pretenden mostrar pases para el análisis.
...
Donde `pass.$(CFBundleIdentifier)` es el identificador del pase que se ha creado [anteriormente](~/ios/platform/passkit.md)
...
el `pass-type-identifiers` matriz derechos es incorrecta (o no coincide con el **perfil de aprovisionamiento**). Compruebe los Pass Type ID y el identificador del equipo son correctas.

## <a name="classes"></a>Clases

Las clases de PassKit siguientes están disponibles para las aplicaciones accedan a pasadas:

-  **PKPass** : es una instancia de un pase.
-  **PKPassLibrary** : proporciona la API para tener acceso a los pases en el dispositivo.
-  **PKAddPassesViewController** – utilizado para mostrar un pase para el usuario guardar en su billetera.
...
Las aplicaciones pueden solicitar una lista de fases de la biblioteca. Esta lista se filtra automáticamente por PassKit, para que solo puedan ver pases que se han creado con el identificador del equipo y que se muestran en los derechos.
...
Un conjunto limitado de información está disponible para la representación de pases de las aplicaciones complementarias.

Elija entre este conjunto de propiedades estándares para mostrar listas de pases, como hace el código de ejemplo.
...
También puede usar el `LocalizedValueForFieldKey()` método para recuperar datos de los campos en los pases que han diseñado (puesto que sabrá qué campos deben estar presente). El código de ejemplo no muestra esto.
...
PKPass no es mutable, por lo que no se puede actualizar los objetos de pase en el código. Para modificar los datos en un pase de una aplicación debe tener acceso a un servidor web que puede mantener un registro de pasadas y generar un nuevo archivo de pase con valores actualizados que puede descargar la aplicación.

Creación del archivo pase debe realizarse en un servidor porque pasadas deben firmarse con un certificado que se debe mantener privada y segura.
...
Las aplicaciones pueden escuchar los cambios realizados en la biblioteca de pases mediante la `PKPassLibraryDidChangeNotification`. La causa de los cambios podrían ser notificaciones desencadenar actualizaciones en segundo plano, por lo que es recomendable escuchar para ellos en la aplicación.
